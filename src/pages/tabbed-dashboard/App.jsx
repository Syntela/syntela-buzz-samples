...
const App = () => {
  ...
  /** @type {React.MutableRefObject<SyntelaBuzz.Dashboard>} */
  ...
  const renderWidget = useCallback((container, { domain, token, hostId }) => {
    ...
    widgetRef.current = new SyntelaBuzz.Dashboard({
      domain,
      sso: {
        token,
        hostId,
    ...
    widgetRef.current.on('dashboard.READY', () => {
      chatWidgetRef.current = new SyntelaBuzz.Dashboard({
        domain,
        container,
      }, SyntelaBuzz.Service.Team);
  ...
  /**
   * @param {SyntelaBuzz.Service} service
   */
  const loadWidget = (service) => {
    widgetRef.current?.toggle(false);
    chatWidgetRef.current?.toggle(false);

    if (service === SyntelaBuzz.Service.None) {
      setIsYourAppVisible(true);
      console.log("Load your app");
    } else if (service === SyntelaBuzz.Service.Team) {
  ...
  return (
    <div className={styles.appContainer}>
      <div className={styles.verticalTabContainer}>
        <button type="button"
          onClick={() => loadWidget(SyntelaBuzz.Service.None)}
        ...
        <button
          type="button"
          onClick={() => loadWidget(SyntelaBuzz.Service.Team)}
          disabled={!chatWidgetReady}
        ...
        <button
          type="button"
          onClick={() => loadWidget(SyntelaBuzz.Service.Drive)}
          disabled={!chatWidgetReady}
        >
          Drive
        </button>
        <button
          type="button"
          onClick={() => loadWidget(SyntelaBuzz.Service.Contacts)}
          disabled={!chatWidgetReady}
        >
          Contacts
        </button>
        <button
          type="button"
          onClick={() => loadWidget(SyntelaBuzz.Service.Meet)}
          disabled={!chatWidgetReady}
        >
          Meetings
        </button>
